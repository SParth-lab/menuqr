  ...
  return (
    <form onSubmit={onSubmit} className="mt-3 max-w-sm space-y-3">
      ...
      <div className="flex items-center gap-3">
        ...
        {status.kind === 'ok' ? <span className="text-xs text-emerald-700">{status.text}</span> : null}
        {status.kind === 'error' ? <span className="text-xs text-red-700">{status.text}</span> : null}
      </div>
    </form>
  );
}
