          ...
          <Card key={t.key} className="p-0">
            ...
            <div className="border-t border-slate-200 p-4">
              <div className="flex items-center justify-between gap-2">
                <h2 className="text-sm font-bold text-slate-900">{t.name}</h2>
                <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-600">
                  {usageByKey.get(t.key) ?? 0} in use
                </span>
              </div>
              <p className="mt-1 text-xs text-slate-500">{t.category}</p>
              <p className="mt-2 text-xs leading-relaxed text-slate-600">{t.description}</p>
              <p className="mt-2 font-mono text-[10px] text-slate-400">{t.key}</p>
            </div>
          </Card>
        ))}
      </div>
