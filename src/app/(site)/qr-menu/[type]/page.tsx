...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `${siteUrl}/qr-menu/${type}` },
    openGraph: { title: post.metaTitle || post.title, description: post.metaDescription || post.excerpt },
  };
}

export default async function QrMenuLanding({ params }: Props) {
  const { type } = await params;
  const post = await getLanding(type);
  if (!post) notFound();
  ...
  return (
    <LandingPage
      ...
      canonical={`${siteUrl}/qr-menu/${type}`}
      crumb="QR menu"
      examples={examples.map((r) => ({ name: r.name, slug: r.slug, city: r.city }))}
      sibling={{ href: `/digital-menu/${type}`, label: `Digital menu for ${type}s` }}
    />
  );
}
